...
CASE(vadd_shift, "vadd.t C001, C000, C100\n")     // d = rows 1-3, s = rows 0-2 of column 0
CASE(vadd_shift2, "vadd.t C000, C001, C100\n")
...
// Cross product. The assembler refuses any overlap for vcrsp, vqmul, vrot and every matrix op
// ("destination register conflict"), so a compiled game can't contain one and they aren't here.
// (vcrsp as raw words is in overlap_vcrsp.c: the hardware doesn't read-before-write there.)
// vmscl with the scalar inside the destination is refused too.
CASE(vcrs_dds,   "vcrs.t C000, C000, C100\n")
CASE(vcrs_ddt,   "vcrs.t C000, C100, C000\n")
CASE(vcrs_ddd,   "vcrs.t C000, C000, C000\n")
CASE(vcrs_shift, "vcrs.t C001, C000, C100\n")

// Unary ops with a shifted destination: the moving-window copy.
CASE(vmov_up,    "vmov.t C001, C000\n")
CASE(vmov_down,  "vmov.t C000, C001\n")
CASE(vmov_row,   "vmov.q R000, C000\n")
CASE(vneg_up,    "vneg.t C001, C000\n")
CASE(vabs_row,   "vabs.p R000, C000\n")
CASE(vsat1_up,   "vsat1.t C001, C000\n")
CASE(vsat0_row,  "vsat0.q R000, C000\n")
CASE(vsgn_up,    "vsgn.t C001, C000\n")
CASE(vocp_up,    "vocp.t C001, C000\n")
...
CASE(vf2h_shift, "vf2h.q C002, C000\n")          // pair out in the upper half of the quad
CASE(vh2f_dd,    "vh2f.p C000, C000\n")          // pair in, quad out
CASE(vh2f_shift, "vh2f.p C000, C002\n")
CASE(vi2c_dd,    "vi2c.q S000, C000\n")
CASE(vi2c_d3,    "vi2c.q S030, C000\n")
CASE(vi2s_dd,    "vi2s.q C000, C000\n")
CASE(vi2s_shift, "vi2s.q C002, C000\n")
CASE(vs2i_dd,    "vs2i.p C000, C000\n")
CASE(vs2i_shift, "vs2i.p C000, C002\n")
...
CASE(vt4444_shift, "vt4444.q C002, C000\n")
...
static const struct { const char *name; const char *text; CaseFunc func; } cases[] = {
	...
	ENTRY(vadd_shift, "vadd.t C001, C000, C100"), ENTRY(vadd_shift2, "vadd.t C000, C001, C100"),
	...
	ENTRY(vcrs_dds, "vcrs.t C000, C000, C100"), ENTRY(vcrs_ddt, "vcrs.t C000, C100, C000"), ENTRY(vcrs_ddd, "vcrs.t C000, C000, C000"), ENTRY(vcrs_shift, "vcrs.t C001, C000, C100"),

	ENTRY(vmov_up, "vmov.t C001, C000"), ENTRY(vmov_down, "vmov.t C000, C001"), ENTRY(vmov_row, "vmov.q R000, C000"),
	ENTRY(vneg_up, "vneg.t C001, C000"), ENTRY(vabs_row, "vabs.p R000, C000"), ENTRY(vsat1_up, "vsat1.t C001, C000"),
	ENTRY(vsat0_row, "vsat0.q R000, C000"), ENTRY(vsgn_up, "vsgn.t C001, C000"), ENTRY(vocp_up, "vocp.t C001, C000"),
	...
	ENTRY(vf2h_dd, "vf2h.q C000, C000"), ENTRY(vf2h_shift, "vf2h.q C002, C000"), ENTRY(vh2f_dd, "vh2f.p C000, C000"), ENTRY(vh2f_shift, "vh2f.p C000, C002"),
	ENTRY(vi2c_dd, "vi2c.q S000, C000"), ENTRY(vi2c_d3, "vi2c.q S030, C000"), ENTRY(vi2s_dd, "vi2s.q C000, C000"), ENTRY(vi2s_shift, "vi2s.q C002, C000"),
	ENTRY(vs2i_dd, "vs2i.p C000, C000"), ENTRY(vs2i_shift, "vs2i.p C000, C002"), ENTRY(vuc2i_dd, "vuc2ifs.s C000, S000"), ENTRY(vuc2i_d3, "vuc2ifs.s C000, S030"),
	...
	ENTRY(vt4444_dd, "vt4444.q C000, C000"), ENTRY(vt5551_dd, "vt5551.q C000, C000"), ENTRY(vt5650_dd, "vt5650.q C000, C000"), ENTRY(vt4444_shift, "vt4444.q C002, C000"),
