...
static short pcmBuf[4608 * 4] __attribute__((aligned(64)));
...
static void testGetMaxOutput(const char *title, int handle) {
	int result = sceMp3GetMaxOutputSample(handle);
	if (result >= 0) {
...
extern "C" int main(int argc, char *argv[]) {
	...
	testGetMaxOutput("  Unreserved", 0);
	handle = sceMp3ReserveMp3Handle(NULL);
	testGetMaxOutput("  NULL info arg", handle);
	sceMp3ReleaseMp3Handle(handle);
	handle = sceMp3ReserveMp3Handle(&mp3Init);
	testGetMaxOutput("  Basic info arg", handle);
	sceMp3ReleaseMp3Handle(handle);
	testGetMaxOutput("  Negative", -1);
	testGetMaxOutput("  2", 2);
	...
	for (int bits = 0; bits < ARRAY_SIZE(channels); ++bits) {
		...
		if (handle < 0) {
			...
			testGetMaxOutput(temp, handle);
		}
		sceMp3ReleaseMp3Handle(handle);
	}

	checkpointNext("Buffer sizes");
	mp3Init.pcmBufSize = sizeof(pcmBuf) / 4;
	handle = initHeader(&mp3Init, baseHeader, sizeof(baseHeader));
	testGetMaxOutput("  Small PCM buffer", handle);
	sceMp3ReleaseMp3Handle(handle);
	mp3Init.pcmBufSize = sizeof(pcmBuf) / 2;
	handle = initHeader(&mp3Init, baseHeader, sizeof(baseHeader));
	testGetMaxOutput("  Double PCM buffer", handle);
	sceMp3ReleaseMp3Handle(handle);
	mp3Init.pcmBufSize = sizeof(pcmBuf);
	handle = initHeader(&mp3Init, baseHeader, sizeof(baseHeader));
	testGetMaxOutput("  Large PCM buffer", handle);
	sceMp3ReleaseMp3Handle(handle);

	handle = sceMp3ReserveMp3Handle(&mp3Init);
	sceMp3TermResource();
	checkpointNext("After term");
	testGetMaxOutput("  Prev allocated handle", handle);
