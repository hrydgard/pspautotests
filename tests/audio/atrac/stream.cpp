...
bool RunAtracTest(const char *filename, AtracTestMode mode, int requestedBufSize, int minRemain, int loopCount, bool enablePlayback) {
	...
	case ATRAC_TEST_HALFWAY:
		...
		if (atracID < 0) {
			...
			schedf("sceAtracSetHalfwayBufferAndGetID: OK, size=%08x (file_size: %08x)\n", load_bytes, at3_size);
		}
		break;
	default:
		schedf("bad mode\n");
	...
	while (!end) {
		...
		if (result) {
			schedf("%i=sceAtracDecodeData error: samples: %08x, end: %08x, remainFrame: %d\n",
				result, samples, end, remainFrame);
			return false;
		}
		schedf("%i=sceAtracDecodeData: samples: %08x, end: %08x, remainFrame: %d\n",
			result, samples, end, remainFrame);
...
extern "C" int main(int argc, char *argv[]) {
	...
	RunAtracTest("sample.at3", ATRAC_TEST_HALFWAY, 0x4000, 10, 0, false);
	RunAtracTest("sample.at3", ATRAC_TEST_FULL, 0, 10, 0, false);
	RunAtracTest("sample.at3", ATRAC_TEST_STREAM, 0x4300, 10, 0, true);
	return 0;
}
