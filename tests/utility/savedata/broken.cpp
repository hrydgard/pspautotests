...
static bool createSave(int secureVersion) {
...
void runBrokenTrials(const char *title, PspUtilitySavedataMode2 mode, int brokenMode) {
	if (brokenMode == 0) {
		checkpointNext(title);
	} else {
		char temp[64];
		snprintf(temp, sizeof(temp), "%s(broken %d):", title, brokenMode);
		checkpointNext(temp);
	}
